...
@section('content')
			...
			$.ajax({
				...
				success : function(result) {
					for(var i = 0; i < result.length; i++){
						$('#enters').append("<li style= 'margin: 100px'> <a href='/Board/detaildetail/"+ result[i].board_id + "'>" + result[i].title + "</a></li>");
					}
				}
			});
			page++;
	...
	function getParam() {
		...
		var sval = "";
		sval = params[params.length-3] + params[params.length-2] + params[params.length-1];
		return sval;
	}
	</script>
@endsection
